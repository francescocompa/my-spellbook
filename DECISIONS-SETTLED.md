# DECISIONS — settled record

> The other half of `DECISIONS.md`, split out 2026-09-07 (`/clean`, completing D158(q)/L5.11)
> so `/start` stops paying ~75k tokens for it every session. **Nothing here is inactive** — a
> settled decision still binds the surface it owns; it simply is not something a session needs
> in hand before it knows what it is working on.
>
> **Read it when** you are about to propose something (check it was not already rejected),
> cite a D-entry, or touch a surface one of these owns. `grep -n "D146" DECISIONS*.md` finds
> any entry in one step; `STATE.md` and `GOTCHAS.md` name the ones that matter per surface.
>
> Same conventions as the live file: a strike-through with `→ D-id` is a supersession; **→
> Gotcha** means `GOTCHAS.md` holds the copy to trust; `→ body: ARCHIVE.md#…` means an earlier
> `/clean` archived the narrative; a cut headline ends `…`.
